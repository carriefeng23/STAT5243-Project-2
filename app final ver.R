        ...
        tabPanel(
          ...
          fluidRow(
            ...
            column(
              ...
              tableOutput("eda_correlation_table"),
              
              br(),
              plotOutput("eda_correlation_plot", height = "400px"),
...
server <- function(input, output, session) {
  ...
  output$eda_correlation_plot <- renderPlot({
    ...
    if (length(num_cols) < 2) {
      plot.new()
      text(0.5, 0.5, "At least two numeric columns are required for correlation plot.")
      return()
    }
    
    corr_mat <- round(cor(df[num_cols], use = "complete.obs"), 4)
  ...
  output$eda_correlation_table <- renderTable({
    df <- eda_data()
    req(df)
